 ...
 ## Generate parameter inputs -----------------------------------------------
  gen_params_psa <- function(n_sim = 1000){
   ...
   df_params_psa <- data.frame(
    ...
    # from dampack::beta_params(mean = 0.721, sigma = 0.05)
    p_AA  = rbeta(n = n_sim, shape1 = 57.29326, shape2 = 22.17034), # transition from health state A to A
    # from dampack::beta_params(mean = 0.202, sigma = 0.05)
    p_AB  = rbeta(n = n_sim, shape1 = 12.82264, shape2 = 50.65576), # transition from health states A to B
    # from dampack::beta_params(mean = 0.067, sigma = 0.05)
    p_AC  = rbeta(n = n_sim, shape1 = 1.608295, shape2 = 22.39611), # transition from health states A to C
    # from dampack::beta_params(mean = 0.010, sigma = 0.05)
    p_AD = rbeta(n = n_sim, shape1 = 0.0296, shape2 = 2.9304), # transition from health states A to Death
    
    ## Transitions from health state B
    # from dampack::beta_params(mean = 0.581, sigma = 0.05)
    p_BB = rbeta(n = n_sim, shape1 = 0.3104903, shape2 = 10.17905), # transition from health states B to B
    # from dampack::beta_params(mean = 0.407, sigma = 0.05)
    p_BC = rbeta(n = n_sim, shape1 = 38.88494, shape2 = 56.65546),  # transition from health states B to C
    # from dampack::beta_params(mean = 0.012, sigma = 0.05)
    p_BD = rbeta(n = n_sim, shape1 = 0.0449088, shape2 = 3.697491), # transition from health states B to Death
    
    ## Transitions from health state C
    # from dampack::beta_params(mean = 0.750, sigma = 0.05)
    p_CC = rbeta(n = n_sim, shape1 = 55.5, shape2 = 18.5),  # transition from health states C to C
    # from dampack::beta_params(mean = 0.250, sigma = 0.05)
    p_CD = rbeta(n = n_sim, shape1 = 18.5, shape2 = 55.5),  # transition from health states C to Death
   ...
   c_direct_state_B = rgamma(n = n_sim, shape = 1774, rate = 1), # direct costs associated with health state B
   c_direct_state_C = rgamma(n = n_sim, shape = 6948, rate = 1), # direct costs associated with health state C
   # Indirect health state costs
   c_indirect_state_A = rgamma(n = n_sim, shape = 1055, rate = 1), # indirect costs associated with health state A
   c_indirect_state_B = rgamma(n = n_sim, shape = 1278, rate = 1), # indirect costs associated with health state B
   c_indirect_state_C = rgamma(n = n_sim, shape = 2059, rate = 1), # indirect costs associated with health state C
   # Drug costs
   c_Trt_1 = c_Trt_1, # cost of SoC treatment (per cycle)
   c_Trt_2 = c_Trt_2 # cost of NT treatment (per cycle)
  )
  
  return(df_params_psa)
}
...
f_MM_hiv <- function(params) {
 with(as.list(params), {
  ...
  a_P_SoC <- array(0,
                   ...
                   dimnames = list(
                    v_names_states, v_names_states, 0:(n_cycles - 1), 1:n_sim))
  a_P_NT <- a_P_SoC
  ...
  # Calculate total costs and LYs per cycle
  for (i in 1:n_sim) {
    # SoC
   m_costs_SoC <- rowSums(t(colSums(a_Y_c_SoC[, , , i]))) # costs per cycle
   m_lys_SoC <- rowSums(t(colSums(a_A_SoC[, c("A", "B", "C"), , i]))) # LYs per cycle
    # New Treatment
   m_costs_NT <- rowSums(t(colSums(a_Y_c_NT[, , , i]))) # costs per cycle
   m_lys_NT <- rowSums(t(colSums(a_A_NT[, c("A", "B", "C"), , i]))) # LYs per cycle
  }
